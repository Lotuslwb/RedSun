...
module.exports = class {
    constructor(page, opt) { // opt 字段有：width, height, 分别代表canvas的宽高，px为单位，
        ...
        this.page = page;
        this.width = opt && opt.width || 400;
        this.height = opt && opt.height ||400;
        this.canvasId = opt && opt.id || 'canvasLock';
        this.endCallback =opt && opt.endCallback ||'';
    ...
    drawCle(x, y) { // 初始化解锁密码面板
      this.ctx.setStrokeStyle('#00a2ff'); // 注意用set
    ...
    bindEvent() {
        ...
        this.page.onTouchend = function (e) {
            if (self.touchFlag) {
                ...
                self.endCallback(self.lastPoint.map((item) => item.index))
                setTimeout(function(){
                    self.reset();
                }, 300);
            }
    ...
    drawPoint() { // 初始化圆心
        for (var i = 0 ; i < this.lastPoint.length ; i++) {
          this.ctx.setFillStyle('#00a2ff'); // 注意用set方法
            this.ctx.beginPath();
